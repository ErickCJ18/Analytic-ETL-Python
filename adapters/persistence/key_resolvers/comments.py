...
def resolver_keys_comments(df_clasificado: pd.DataFrame, engine: Engine) -> CleaningResult:
    verificar_dims_cargadas(engine, DIMS_REQUERIDAS_COMMENTS)

    logger.info(f"Iniciando resolución de surrogate keys para FactComments | filas={len(df_clasificado)}")
    ...
    if len(df_rechazadas) > 0:
        ...
        logger.warning(f"{len(df_rechazadas)} filas de FactComments sin match en alguna dimensión")

    df_validas = df[~condiciones_sin_match].copy()

    df_validas = df_validas[[
        "ClienteKey", "ProductoKey", "OrigenKey", "FechaKey", "ClasificacionKey", "Comentario"
    ]].copy()

    resultado = CleaningResult("FactComments_Keys", df_validas, df_rechazadas, procesadas)
    logger.info(resultado.resumen())

    return resultado


class FactCommentsKeyResolver(SurrogateKeyResolver):
    def __init__(self, engine: Engine):
        self.engine = engine

    def resolve(self, df: pd.DataFrame) -> CleaningResult:
        return resolver_keys_comments(df, self.engine)
