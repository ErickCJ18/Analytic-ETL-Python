...
def resolver_keys_opiniones(df_limpio: pd.DataFrame, engine: Engine) -> CleaningResult:
    verificar_dims_cargadas(engine, DIMS_REQUERIDAS_OPINIONES)

    logger.info(f"Iniciando resolución de surrogate keys para FactOpiniones | filas={len(df_limpio)}")
    df = df_limpio.copy()
    procesadas = len(df)
    ...
    df = df.merge(dim_origen, left_on="Fuente", right_on="NombreFuente", how="left")
    ...
    if len(df_rechazadas) > 0:
        ...
        logger.warning(f"{len(df_rechazadas)} filas de FactOpiniones sin match en alguna dimensión")

    df_validas = df[~condiciones_sin_match].copy()

    df_validas = df_validas[[
        "ClienteKey", "ProductoKey", "OrigenKey", "FechaKey", "ClasificacionKey",
        "Comentario", "PuntajeSatisfacción"
    ]].rename(columns={"PuntajeSatisfacción": "PSatisfaccion"})

    resultado = CleaningResult("FactOpiniones_Keys", df_validas, df_rechazadas, procesadas)
    logger.info(resultado.resumen())

    return resultado


class FactOpinionesKeyResolver(SurrogateKeyResolver):
    def __init__(self, engine: Engine):
        self.engine = engine

    def resolve(self, df: pd.DataFrame) -> CleaningResult:
        return resolver_keys_opiniones(df, self.engine)
